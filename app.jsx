// Norsk A1 — App de examen · main app  (v1.1.0)
...
// True inside the Claude Design host (iframe); false when deployed/opened
// directly (GitHub Pages, phone). Drives frame-vs-fullscreen rendering.
const EMBEDDED = (() => { try { return window.self !== window.top; } catch { return true; } })();

// ─── Design tokens (H2SITE) ────────────────────────────────────────────────
const T = {
  bg: '#F4F6F8',
  surface: '#ffffff',
  surfaceAlt: '#F0F3F5',
...
T.accent = T.navy;
T.accentLight = T.navyLight;
T.accentInk = T.navyInk;

// ─── Sound (Web Audio, synthesized — soft & subtle) ────────────────────────
const SoundFX = (() => {
  let ctx = null;
  const ac = () => {
    if (!ctx) {
...
// ─── Icons (inline SVG — no emoji, per brand) ──────────────────────────────
  ...
  return (
    <button onClick={onClick} disabled={disabled}
      style={{
        ...
        font: `700 15px ${FD}`, letterSpacing: '0.005em',
        cursor: disabled ? 'not-allowed' : 'pointer',
        opacity: disabled ? 0.4 : 1, width: full ? '100%' : 'auto',
        display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 8,
        transition: 'transform 0.08s ease, opacity 0.15s ease', ...style,
      }}
...
function Pill({ children, color = T.text2, bg = T.surfaceAlt, style }) {
  return (
    <span style={{
      ...
      padding: '4px 9px', borderRadius: 6, background: bg, color,
      font: `700 10.5px ${FB}`, textTransform: 'uppercase', letterSpacing: '0.06em', ...style,
    }}>{children}</span>
  );
}
...
function SoundToggle({ on, onToggle, dark }) {
  return (
    <button onClick={onToggle} aria-label={on ? 'Apagar sonido' : 'Encender sonido'}
      style={{
        background: dark ? 'rgba(255,255,255,0.14)' : T.surfaceAlt,
...
function SourceRef({ source, tone = 'navy' }) {
  ...
  return (
    <div style={{
      ...
      background: bg, color: col, borderRadius: 8, padding: '6px 10px',
      font: `600 11.5px ${FB}`, marginTop: 10, lineHeight: 1.3,
    }}>
...
// ─── Home ─────────────────────────────────────────────────────────────────
function HomeScreen({ mode, setMode, selectedTopics, examSize, setExamSize, soundOn, setSoundOn, onStart, onOpenTopics }) {
  const totalQs = window.ALL_QUESTIONS.length;
  const available = window.ALL_QUESTIONS.filter(q => selectedTopics.includes(q.topic)).length;
  const canStart = available >= 1 && selectedTopics.length >= 1;

  const presets = [10, 15, 20, 25, 30].filter(n => n <= available);
    ...
    <div style={{ padding: '20px 20px 28px', display: 'flex', flexDirection: 'column', gap: 20 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        <div>
          ...
          <p style={{ font: `400 13.5px/1.5 ${FB}`, color: T.text2, margin: 0 }}>
            Practica el examen de noruego A1, pregunta a pregunta.
          </p>
        </div>
      ...
      <div>
        ...
        <button onClick={onOpenTopics}
          style={{
            background: T.surface, border: `1.5px solid ${T.border}`, width: '100%',
            padding: '13px 14px', borderRadius: 13, textAlign: 'left',
            cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 12,
          }}>
          <div style={{ width: 38, height: 38, borderRadius: 10, background: T.navyLight, color: T.navy, display: 'grid', placeItems: 'center', font: `800 16px ${FD}` }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ font: `400 12px ${FB}`, color: T.text2, marginTop: 1 }}>
              {available} preguntas disponibles · de {totalQs}
            </div>
          </div>
      ...
      <div>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
          ...
          <CountChip value={available} label={`Todas · ${available}`} active={effectiveSize === available && !presets.includes(effectiveSize)} onClick={() => setExamSize(available)} />
        </div>
      </div>

      <Btn full variant={mode === 'exam' ? 'primary' : 'teal'} onClick={onStart} disabled={!canStart}>
      ...
      <div style={{ font: `400 11px ${FB}`, color: T.text3, textAlign: 'center', letterSpacing: '0.02em' }}>
        Norsk A1 · App de examen · {window.APP_VERSION}
      </div>
    </div>
  );
}
  ...
  return (
    <button onClick={onClick}
      style={{
        ...
        font: `700 13.5px ${FD}`, minWidth: 46,
...
function ModeCard({ active, onClick, label, description, icon }) {
  return (
    <button onClick={onClick}
      style={{
        background: active ? T.tealPale : T.surface,
      ...
      <div style={{
        ...
        background: active ? T.teal : T.surfaceAlt, color: active ? '#fff' : T.navy,
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 9 }}>
          ...
          {active && <Pill color={T.teal} bg="rgba(60,179,170,0.16)">Activo</Pill>}
        </div>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      <div style={{ padding: '12px 20px', display: 'flex', flexDirection: 'column', gap: 7, flex: 1 }}>
        <button onClick={toggleAll} style={{ background: T.surface, border: `1.5px solid ${T.border}`, borderRadius: 11, padding: '11px 13px', cursor: 'pointer', font: `700 13px ${FD}`, color: T.navy, textAlign: 'left', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        {topics.map(topic => {
          ...
          return (
            <button key={topic.id} onClick={() => toggle(topic.id)}
              style={{ background: checked ? T.tealPale : T.surface, border: `1.5px solid ${checked ? T.teal : T.border}`, borderRadius: 13, padding: '12px 13px', textAlign: 'left', cursor: 'pointer', display: 'flex', gap: 12, alignItems: 'flex-start' }}>
              <Checkbox checked={checked} />
              <div style={{ flex: 1 }}>
                <div style={{ display: 'flex', alignItems: 'baseline', gap: 8 }}>
                  <span style={{ font: `800 14px ${FD}`, color: T.navy }}>{topic.name}</span>
                  <span style={{ font: `700 11px ${FB}`, color: T.text3 }}>· {counts[topic.id]} preg.</span>
                </div>
...
function Checkbox({ checked }) {
  return (
    <div style={{ width: 22, height: 22, borderRadius: 6, border: `1.5px solid ${checked ? T.teal : T.border}`, background: checked ? T.teal : T.surface, display: 'grid', placeItems: 'center', flexShrink: 0, marginTop: 1 }}>
...
function ExamScreen({ mode, questions, soundOn, setSoundOn, onFinish, onAbort }) {
  ...
  const pick = (optIdx) => {
    ...
    if (mode === 'study') {
      ...
      if (soundRef.current) (correct ? SoundFX.correct() : SoundFX.wrong());
    } else {
      // Exam: neutral tick only (no right/wrong reveal), then auto-advance.
      if (soundRef.current) SoundFX.tick();
      const next = [...answers, entry];
      setAnswers(next);
      setTimeout(() => goNext(next), 230);
    }
  };
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      <div style={{ flex: 1, padding: '17px 20px 16px', display: 'flex', flexDirection: 'column', gap: 13, overflow: 'auto' }}>
        ...
        <div style={{ fontFamily: FD, fontWeight: 800, fontSize: 'var(--qscale, 19px)', lineHeight: 1.38, color: T.text, textWrap: 'pretty', letterSpacing: '-0.003em' }}>{q.q}</div>
        ...
        {revealed && (
          <div style={{ background: selected === q.correct ? T.successLight : T.errorLight, border: `1px solid ${selected === q.correct ? T.success : T.error}33`, borderRadius: 13, padding: '13px 15px', marginTop: 4 }}>
            <div style={{ font: `800 11px ${FB}`, textTransform: 'uppercase', letterSpacing: '0.07em', color: selected === q.correct ? T.success : T.error, marginBottom: 6, display: 'inline-flex', alignItems: 'center', gap: 6 }}>
              <Icon name={selected === q.correct ? 'check' : 'x'} size={14} color={selected === q.correct ? T.success : T.error} stroke={2.4} />
...
function labelType(t) {
  return ({ vocab: 'Vocabulario', grammar: 'Gramática', noun: 'Sustantivos', phrase: 'Frases', numbers: 'Números' })[t] || t;
}
...
function OptionRow({ letter, label, selected, correct, wrong, dim, onClick, disabled }) {
  ...
  return (
    <button onClick={onClick} disabled={disabled}
      style={{ background: bg, border: `1.5px solid ${border}`, borderRadius: 13, padding: '12px 13px', textAlign: 'left', cursor: disabled ? 'default' : 'pointer', display: 'flex', gap: 12, alignItems: 'flex-start', opacity: dim ? 0.5 : 1, transition: 'all 0.15s ease' }}>
...
function ResultsScreen({ questions, answers, mode, timeUsed, soundOn, onRestart, onHome }) {
  ...
  useEffect(() => {
    if (soundOn) (passed ? SoundFX.win() : SoundFX.fail());
  }, []);
        ...
        <div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {questions.map((q, i) => {
              ...
              return (
                <div key={q.id} style={{ background: T.surface, border: `1px solid ${T.borderSubtle}`, borderRadius: 11, overflow: 'hidden' }}>
                  <button onClick={() => setExpandedQ(exp ? null : i)} style={{ background: 'none', border: 'none', width: '100%', padding: '11px 13px', textAlign: 'left', cursor: 'pointer', display: 'flex', alignItems: 'flex-start', gap: 10 }}>
                    <div style={{ width: 24, height: 24, borderRadius: 6, flexShrink: 0, background: isCorrect ? T.success : (a && a.selected !== null ? T.error : T.text3), color: '#fff', display: 'grid', placeItems: 'center', marginTop: 1 }}>
                      <Icon name={isCorrect ? 'check' : 'x'} size={13} color="#fff" stroke={2.6} />
                    </div>
                    <div style={{ flex: 1 }}>
                      <div style={{ font: `700 11px ${FB}`, color: T.text3, marginBottom: 3 }}>Pregunta {i + 1} · {window.TOPICS.find(t=>t.id===q.topic).name}</div>
                  ...
                  {exp && (
                    <div style={{ padding: '0 13px 13px', display: 'flex', flexDirection: 'column', gap: 7 }}>
                      {q.options.map((opt, oi) => {
                        ...
                        return (
                          <div key={oi} style={{ padding: '8px 10px', borderRadius: 8, background: isAnswer ? T.successLight : (wasPicked ? T.errorLight : T.surfaceAlt), border: `1px solid ${isAnswer ? T.success+'55' : (wasPicked ? T.error+'55' : T.borderSubtle)}`, font: `500 12px/1.4 ${FB}`, color: T.text, display: 'flex', gap: 8, alignItems: 'flex-start' }}>
...
function App() {
  ...
  useEffect(() => {
    try { localStorage.setItem('norsk_sound', soundOn ? 'on' : 'off'); } catch {}
  }, [soundOn]);
  ...
  const startExam = () => {
    ...
    setExamQuestions(shuffle(pool).slice(0, n));
  ...
  let content;
  if (screen === 'home') {
    content = <HomeScreen mode={mode} setMode={setMode} selectedTopics={selectedTopics} examSize={examSize} setExamSize={setExamSize} soundOn={soundOn} setSoundOn={setSoundOn} onStart={startExam} onOpenTopics={() => setScreen('topics')} />;
  ...
  // Standalone (deployed) vs embedded (Claude Design host iframe). When the page
  // is opened directly — e.g. on GitHub Pages or a phone — render the app
  // full-screen. Inside the design host it stays in the phone frame with tweaks.
  if (!EMBEDDED) {
    return (
      <div className="app-root" data-screen-label={`Norsk A1 · ${screen}`}
  ...
  return (
    <div className="stage">
      <div data-screen-label={`Norsk A1 · ${screen}`} className="app-root" style={{ '--qscale': `${tw.questionScale}px` }}>
      ...
      <div className="annotation">
        <h4>Norsk A1 · App de examen · {window.APP_VERSION}</h4>
        <p style={{ margin: 0 }}>
          <strong>Modo Estudio:</strong> respuesta correcta y explicación tras cada pregunta, con la fecha de los apuntes donde repasar.<br /><br />
          <strong>Modo Examen:</strong> sin pistas; resultado y corrección completa solo al final.<br /><br />
          Banco: <strong>{window.ALL_QUESTIONS.length}</strong> preguntas · <strong>{window.TOPICS.length}</strong> kapitler. Sonido conmutable.
        </p>
      </div>
